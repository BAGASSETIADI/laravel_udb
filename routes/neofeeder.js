...
var MataKuliahkurikulum = require('../models/MataKuliahKurikulum');
...
router.get('/profil', cekToken, async function(req, res, next) {
    ...
    if (hasil.data.error_code == 0) {
        ...
        var dataList = await Promise.all(dataRes.map((item) => {
            return {
        ...
        SatuanPendidikan.bulkCreate(
            dataList, { updateOnDuplicate: ["nama_perguruan_tinggi"] }
        );
    }
    res.json(hasil.data);
});
...
router.get('/jenjangpendidikan', cekToken, async function(req, res, next) {
    ...
    if (hasil.data.error_code == 0) {
        ...
        JenjangPendidikan.bulkCreate(
            dataList, { updateOnDuplicate: ["nama_jenj_didik"] }
        );
    }
    res.json(hasil.data);
});
    ...
    if (hasil.data.error_code == 0) {
        ...
        var dataList = await Promise.all(dataRes.map((item) => {
            return {
                ...
                id_neofeder: item.id_prodi
            }
        }));
        //tambah atau update data tabel jenjang_pendidikan
        ProgramStudi.bulkCreate(
            dataList, { updateOnDuplicate: ["nama_prodi", "id_jenj_didik"] }
        );
    }
    res.json(hasil.data);
});
...
router.get('/kurikulum', cekToken, async function(req, res, next) {
    ...
    //cek apakah tidak ada eror maka update atau create tabel Kurikulum
    if (hasil.data.error_code == 0) {
        //ambil nilai response data
        var dataRes = hasil.data.data;

        //membuat variable dataList untuk di-insert ke tabel
        var dataList = await Promise.all(dataRes.map(async(item) => {
            var dataProdi = await ProgramStudi.findByPk(item.id_prodi);
            var dataProdi = await ProgramStudi.findAll({
                where: { id_neofeder: item.id_prodi }
            ...
            return {
                ...
                id_neofeeder: item.id_kurikulum
            }
        }));
        //tambah atau update data tabel jenjang_pendidikan
        Kurikulum.bulkCreate(
            dataList, { updateOnDuplicate: ["nama_kurikulum"] }
        );
    }
    res.json(hasil.data);
});
...
router.post('/matakuliah', cekToken, async function(req, res, next) {
    ...
    var responList = await Promise.all(data.map(async(item) => {
        ...
        var hasil = await axios.post(
            url_neo, {
                ...
                record: {
                    ...
                    id_prodi: dataProdi.id_neofedeer,
        ...
        if (hasil.data.error_code == 0) {
            await MataKuliah.update({
                id_neofedeer: hasil.data.data.id_matkul,
